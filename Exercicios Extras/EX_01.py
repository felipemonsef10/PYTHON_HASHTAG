...
def criar_lista():
    ...
    lista_de_compras = set(lista_de_compras)
...
def exclui_item(lista, item):
    lista.remove(item)
    return lista


def vizualisa_lista(lista):
...
while True:
    ...
    if opcao == '1':
        ...
        item = str(input('Item: '))
        lista = adiciona_item(lista, item)
        print('ITEM ADICIONADO COM SUCESSO!')
    if opcao == '2':
        print('====== Removendo Item ======')
        item = str(input('Item: ')).strip()
        lista = exclui_item(lista, item)
        print('ITEM REMOVIDO COM SUCESSO!')
    if opcao == '3':
        print('====== Visualizando Itens ======')
        vizualisa_lista(lista)
